...
func Init(c context.Context) (*Executor, error) {
	...
	ag, err := agent.Init(c)
	if err != nil {
		logger.Error(c, "agent_init_err", logger.C(err))
		return nil, err
	}
	...
	if cfg.Path.SnapshotEnabled {
		...
		if err := snapSvc.InitBaseline(); err != nil {
			logger.Warn(c, "init_snapshot_err", logger.C(err))
			snapSvc = nil
		} else {
			logger.Info(c, "init_snapshot_ok", logger.S("baseDir", baseDir))
		}
	} else {
		logger.Info(c, "snapshot_stop")
	}
