...
func (h *Humen) createCustomAgents(ctx context.Context, reg *llm.Registry, mmModel model.BaseChatModel) ([]adk.Agent, []AgentInfo, error) {
	var subagents []adk.Agent
	var infos []AgentInfo

	for _, cfg := range conf.GetConfig().Agents {
		agentTools, err := tools.NewWithName(cfg.Tools, mmModel, h.ragSvc)
		if err != nil {
...
func (h *Humen) createMiferAgent(ctx context.Context, reg *llm.Registry, mmModel model.BaseChatModel, subagents []adk.Agent) (adk.Agent, AgentInfo, error) {
	...
	info := AgentInfo{Name: "Mifer", ModelBackend: "default", Description: "Mifer 智能助手，具备文件操作、命令执行、知识库管理、网页搜索和 QQ 消息等完整能力", Tools: resolveToolNames(ctx, orchTools)}
